// Script to directly add test images to a property
require('dotenv').config();
const { PrismaClient } = require('@prisma/client');
const { createClient } = require('@supabase/supabase-js');
const fs = require('fs');
const path = require('path');

// Configuration
const PROPERTY_ID = 22; // Property 3 in UI
const IMAGES_TO_ADD = 2;
const BUCKET_NAME = 'property-images'; // Hardcoded bucket name

// Set up clients
const prisma = new PrismaClient();
const supabase = createClient(
  process.env.SUPABASE_URL,
  process.env.SUPABASE_SERVICE_KEY,
  { auth: { persistSession: false } }
);

// Create a test image
const createTestImage = (index) => {
  // Create a simple text file as a test placeholder
  const testDir = path.join(__dirname, 'temp');
  if (!fs.existsSync(testDir)) {
    fs.mkdirSync(testDir, { recursive: true });
  }

  const imagePath = path.join(testDir, `test-image-${index}.txt`);
  fs.writeFileSync(
    imagePath,
    `This is test image ${index} - ${new Date().toISOString()}`
  );

  return {
    path: imagePath,
    buffer: fs.readFileSync(imagePath),
    name: `test-image-${index}.txt`,
  };
};

const addImagesToProperty = async () => {
  try {
    console.log(`Adding test images to property ${PROPERTY_ID}...`);

    // Check if property exists
    const property = await prisma.property.findUnique({
      where: { id: PROPERTY_ID },
      select: {
        id: true,
        name: true,
        images: true,
        photoUrls: true,
      },
    });

    if (!property) {
      console.error(`Property ${PROPERTY_ID} not found`);
      return false;
    }

    console.log(`Found property: ${property.name}`);
    console.log(`Current images: ${property.images?.length || 0}`);
    console.log(`Current photoUrls: ${property.photoUrls?.length || 0}`);

    // Log the bucket info
    console.log(`Using bucket: ${BUCKET_NAME}`);

    // List buckets to verify
    const { data: buckets } = await supabase.storage.listBuckets();
    console.log('Available buckets:', buckets.map((b) => b.name).join(', '));

    // Create test images and upload them
    const uploadedUrls = [];

    for (let i = 0; i < IMAGES_TO_ADD; i++) {
      // Create test image
      const image = createTestImage(i + 1);
      console.log(`Created test image: ${image.name}`);

      // Upload to Supabase
      const fileName = `${PROPERTY_ID}/${Date.now()}-${image.name}`;
      console.log(`Uploading to ${BUCKET_NAME}/${fileName}...`);

      const { data, error } = await supabase.storage
        .from(BUCKET_NAME)
        .upload(fileName, image.buffer, {
          contentType: 'text/plain',
          upsert: true,
        });

      if (error) {
        console.error(`Failed to upload image ${i + 1}:`, error);
        continue;
      }

      // Get public URL
      const { data: urlData } = supabase.storage
        .from(BUCKET_NAME)
        .getPublicUrl(fileName);

      console.log(`Upload successful, URL: ${urlData.publicUrl}`);
      uploadedUrls.push(urlData.publicUrl);

      // Clean up temporary file
      fs.unlinkSync(image.path);
    }

    if (uploadedUrls.length === 0) {
      console.error('Failed to upload any images');
      return false;
    }

    // Update property record
    const existingImages = property.images || [];
    const existingPhotoUrls = property.photoUrls || [];

    await prisma.property.update({
      where: { id: PROPERTY_ID },
      data: {
        images: [...existingImages, ...uploadedUrls],
        photoUrls: [...existingPhotoUrls, ...uploadedUrls],
      },
    });

    console.log(`Updated property with ${uploadedUrls.length} new images`);
    console.log('Image URLs:', uploadedUrls);

    // Verify update
    const updatedProperty = await prisma.property.findUnique({
      where: { id: PROPERTY_ID },
      select: {
        id: true,
        images: true,
        photoUrls: true,
      },
    });

    console.log('Updated property:');
    console.log(`- Images: ${updatedProperty.images?.length || 0}`);
    console.log(`- PhotoUrls: ${updatedProperty.photoUrls?.length || 0}`);

    return true;
  } catch (err) {
    console.error('Error adding images:', err);
    return false;
  } finally {
    await prisma.$disconnect();
  }
};

addImagesToProperty()
  .then((success) => {
    console.log(`\nOperation ${success ? 'COMPLETED' : 'FAILED'}`);
    process.exit(success ? 0 : 1);
  })
  .catch((err) => {
    console.error('Fatal error:', err);
    process.exit(1);
  });
